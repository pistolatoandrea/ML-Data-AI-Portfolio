...
- **Streamlit UI** — real-time visibility into agent reasoning and steps
...
│   ├── tools.py        # Tavily search tool + file writer tool
...
### 4. Writer
Synthesizes all research findings into a structured markdown report with Executive Summary, Key Findings, and Conclusions. Saves the output to `outputs/research_report.md` using the file writer tool.
...
Enter your research task in the input field and watch the agent work through each step in real time. The final report will appear in the UI and be saved to `outputs/research_report.md`.
...
**Agent steps:**
1. Planner generates 3 search queries
2. Searcher retrieves results from Tavily for each query
3. Evaluator reads results → `insufficient` → second search iteration
4. Evaluator reads expanded results → `sufficient`
5. Writer produces structured markdown report → saved to `outputs/`
...
- Best practices for **API key management** in Python projects
- How to structure a multi-file Python agent project cleanly

---

*Part of the [ML-Data-AI Portfolio](https://github.com/pistolatoandrea/ML-Data-AI-Portfolio) — a 10-project end-to-end learning path through Machine Learning, MLOps, and Modern AI.*